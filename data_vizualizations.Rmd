...
```{r}
taxi_2016_df <- as.data.frame(taxi_2016)
taxi_2015_df <- as.data.frame(taxi_2015)
...
taxi_2016 <- read.csv (here::here("data", "2016_Taxi_Trips.csv"))[,3]
taxi_2016_other <- read.csv (here::here("data", "2016_Taxi_Trips1.csv"))[,1]
taxi_2017 <- read.csv (here::here("data", "2017_Taxi_Trips.csv"))[,3]
taxi_2017_other <- read.csv (here::here("data", "2017_Taxi_Trips1.csv"))[,1]

taxi_start_times <- mdy_hms(fct_c(taxi_2015, taxi_2017))
...
total_demand_by_day %>%
  ...
  annotate("text", x = as.Date("2017-04-10"), y = 30, 
...
mini_divvy <- divvy_from_sf[1:500,]
...
ggplot(data = x[1:500,]) +
  geom_link(aes(y = Pickup.Centroid.Latitude, x = Pickup.Centroid.Longitude, 
                yend = Dropoff.Centroid.Latitude, xend = Dropoff.Centroid.Longitude, 
                colour = Payment.Type))
```
...
st_linestring(x = matrix((mini$Pickup.Centroid.Longitude, mini$Pickup.Centroid.Latitude,
                                 mini$Dropoff.Centroid.Longitude, mini$Dropoff.Centroid.Latitude)
                         , 0, 2), dim = "XY")


multipoint_sfg <- st_multipoint(rbind(c(, 34.1168926), c(4.8979755, 52.3745403)))


linestring_sfg <- st_linestring(rbind(c(mini$Pickup.Centroid.Longitude, mini$Pickup.Centroid.Latitude), c(mini$Dropoff.Centroid.Longitude, mini$Dropoff.Centroid.Latitude)))
...
mini_taxi$Pickup.Centroid.Latitude = as.numeric(as.character(mini_taxi$Pickup.Centroid.Latitude))
mini_taxi$Dropoff.Centroid.Latitude = as.numeric(as.character(mini_taxi$Dropoff.Centroid.Latitude))
mini_taxi$Dropoff.Centroid.Longitude = as.numeric(mini_taxi$Dropoff.Centroid.Longitude)
...
ggplot(y, aes(x=Pickup.Centroid.Longitude, y=Pickup.Centroid.Latitude, size = num_trips, colour = median_cost)) +
  geom_point() +
